...
namespace atomic_dex
{
    ...
    wallet_page::send(const QString& address, const QString& amount, bool max, bool with_fees, QVariantMap fees_data)
    {
        ...
        //! Process
        auto error_functor = [&json_data](pplx::task<void> previous_task)
        { handle_exception_pplx_task(previous_task, fmt::format("send: {}", json_data.dump(4))); };
    ...
    wallet_page::broadcast_on_auth_finished(bool is_auth, const QString& tx_hex, bool is_claiming, bool is_max, const QString& amount)
    {
        ...
        auto error_functor = [&json_data](pplx::task<void> previous_task)
        { handle_exception_pplx_task(previous_task, fmt::format("broadcast_on_auth_finished: {}", json_data.dump(4))); };
        mm2_system.get_mm2_client().async_rpc_batch_standalone(batch).then(answer_functor).then(error_functor);
    }
    ...
    wallet_page::claim_rewards()
    {
        ...
        json_data["userpass"] = "";
        auto error_functor = [&json_data](pplx::task<void> previous_task)
        { handle_exception_pplx_task(previous_task, fmt::format("kmd_rewards_info: {}", json_data.dump(4))); };
    ...
    wallet_page::claim_faucet()
    {
        ...
        auto error_functor = [&claim_request](pplx::task<void> previous_task)
        { handle_exception_pplx_task(previous_task, fmt::format("claim_faucet: {}", claim_request.coin_name)); };
